...
const AlphabetOnlyKeypad: React.FC<KeyPadProps> = ({ onKeyPress, colorMode }) => {
  ...
  return <div style={containerStyle}>
    <div style={styles.keypadContainer}>
      ...
      <div style={styles.enterBtn}>
        <button style={btnStyle} onClick={_ => handleKeyPress(SpecialKeys.Enter)}>
          ENTER
        </button>
      </div>
      <div
        style={styles.cancelBtn}
        onClick={_ => handleKeyPress(SpecialKeys.Backspace)}>
      ...
      {alphaKeys.map((row, i) => row.split('')
        .map(ch => <button
          key={ch}
          style={{ ...btnStyle, ...styles[`alphaRow${i + 1}`] }}
